...
public final class PuppyClickerConfigScreen extends Screen {
    ...
    protected void init() {
        ...
        // The formatter changes presentation only; getValue still returns the real draft.
        apiKeyField.addFormatter((value, offset) -> masked
    ...
    public void extractRenderState(
        ...
        int contentWidth = Math.min(CONTENT_WIDTH, this.width - 32);
        int left = (this.width - contentWidth) / 2;
        graphics.centeredText(this.font, this.title, this.width / 2, 28, 0xFFFFFF);
        graphics.text(
                this.font,
                Component.translatable("screen.puppyclicker.config.api_key"),
                left,
        ...
        MultiLineLabel.create(this.font, this.status, contentWidth)
                .visitLines(TextAlignment.CENTER, this.width / 2, 100, 9, graphics.textRenderer());
